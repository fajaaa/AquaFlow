...
import 'package:aquaflow_desktop/admin/models/admin_support_ticket_photo.dart';
...
/// Chat-thread detail of one support ticket, pushed from
/// `AdminSupportTicketsScreen` as its own Scaffold+AppBar route (same
/// `context.pushScreen` pattern as `AdminUserActivityLogsScreen`).
///
/// This is the same chat thread the customer sees on
/// `CustomerSupportTicketDetailScreen` (`GET /SupportTickets/{id}`, full
/// `Messages` thread): bubbles render the admin's own staff replies on the
/// right and the customer's messages on the left (inverse of the customer
/// app's alignment, where the signed-in user's own messages are on the
/// right). A reply composer (text + photo attachments, desktop file picker
/// instead of `image_picker`) sits at the bottom while the ticket is Open;
/// `IsFromStaff` is derived server-side from the caller's
/// `SupportTickets.Manage` permission, never sent by this screen. The app bar
/// carries the "Zatvori"/"Ponovo otvori" actions
/// (`POST /SupportTickets/{id}/close|reopen`) instead of a status-derived
/// composer swap, since staff may want to close a ticket without waiting for
...
class _AdminSupportTicketDetailScreenState
  ...
  // Uses image_picker's gallery source rather than a camera/gallery choice
  // sheet like the mobile customer composer (`NewSupportTicketDialog`) -
  // there is no camera to offer on desktop, and image_picker's gallery source
  // already opens the native file picker on Windows (image_picker_windows).
  Future<void> _pickImages() async {
    if (_selectedImages.length >= _maxPhotosPerMessage) return;
  ...
  Future<void> _toggleStatus() async {
    final ticket = _ticket;
    if (ticket == null || _togglingStatus) return;

    setState(() => _togglingStatus = true);
    try {
      final updated = ticket.isClosed
          ? await _service.reopen(ticket.id)
          : await _service.close(ticket.id);
      if (!mounted) return;
      setState(() {
        // The transition endpoints return the header only - keep the
        // already-loaded thread rather than dropping it.
        _ticket = AdminSupportTicket(
          id: updated.id,
          ...
          lastMessageFromStaff: updated.lastMessageFromStaff,
  ...
  void _openFullscreen(int messageId, AdminSupportTicketPhoto photo) {
    context.pushScreen(
  ...
  Widget _buildBody() {
    ...
    return Column(
      children: [
        ...
        if (ticket.isClosed) const _ClosedBanner() else _buildComposer(),
      ],
    );
  }

  Widget _buildThread(AdminSupportTicket ticket) {
...
class _MessageBubble extends StatelessWidget {
  ...
  final AdminSupportTicketMessage message;
  final void Function(AdminSupportTicketPhoto photo) onPhotoTap;
  final Future<Uint8List> Function(AdminSupportTicketPhoto photo) photoBytes;

  @override
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    // The admin's own (staff) messages on the right, the customer's on the
    // left - inverse of the customer app's alignment.
    final fromStaff = message.isFromStaff;
    final alignment = fromStaff ? Alignment.centerRight : Alignment.centerLeft;
