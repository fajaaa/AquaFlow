...
public class AssignedWaterMeterRequestState : BaseWaterMeterRequestState
{
    ...
    // meter under a different customer); SettlementId comes from meterData, which
    // WaterMeterRequestService.RegisterAsync already resolved from that same customer's profile.
    // The remaining fields come from meterData. The new meter, the ResultingWaterMeterId backlink,
    // the status change and the history row all commit in the single SaveChanges inside
    // TransitionToAsync.
