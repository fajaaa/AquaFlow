...
namespace AquaFlow.WebAPI.Tests.ConsumptionAlerts;

// /WaterConsumptionAlerts is admin-only in full (see the controller): the gate is declared once at
// class level rather than per-action, so the reads and Recompute are covered too. These tests pin
// both halves of that - the attribute is present with the right code, and the filter it declares
// really does return 403 for an authenticated caller without the code, on every action the
// controller exposes.
public class WaterConsumptionAlertsControllerTests
{
    private const string ManagePermission = "ConsumptionAlerts.Manage";
    ...
    [
        nameof(WaterConsumptionAlertsController.GetAll),
        nameof(WaterConsumptionAlertsController.GetById),
        nameof(WaterConsumptionAlertsController.Create),
        nameof(WaterConsumptionAlertsController.Update),
        nameof(WaterConsumptionAlertsController.Patch),
        nameof(WaterConsumptionAlertsController.Delete),
        nameof(WaterConsumptionAlertsController.Recompute)
    ];
    ...
    // silently reopening consumption alerts to any authenticated caller.
    [Fact]
    public void Controller_RequiresConsumptionAlertsManagePermission()
    {
        var attribute = typeof(WaterConsumptionAlertsController)
    ...
    private static AuthorizationFilterContext AuthorizeAction(string methodName, ClaimsPrincipal user)
    {
        ...
        var context = new AuthorizationFilterContext(
            new ActionContext(
                ...
                {
                    ControllerTypeInfo = typeof(WaterConsumptionAlertsController).GetTypeInfo(),
    ...
    // none of them); Recompute is overloaded (parameterless and int-taking), so the first match by
    // name is enough since the filter only cares about the action name, not its parameters.
    private static MethodInfo ResolveAction(string methodName)
    {
        return typeof(WaterConsumptionAlertsController)
