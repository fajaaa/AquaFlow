...
namespace AquaFlow.WebAPI.Tests.Recommendations;

// /Recommendations is admin-only in full (see the controller): the gate is declared once at class
// level rather than per-action, so the reads and Recompute are covered too. These tests pin both
// halves of that - the attribute is present with the right code, and the filter it declares really
// does return 403 for an authenticated caller without the code, on every action the controller exposes.
public class RecommendationsControllerTests
{
    private const string ManagePermission = "Recommendations.Manage";
    ...
    [
        nameof(RecommendationsController.GetAll),
        nameof(RecommendationsController.GetById),
        nameof(RecommendationsController.Create),
        nameof(RecommendationsController.Update),
        nameof(RecommendationsController.Patch),
        nameof(RecommendationsController.Delete),
        nameof(RecommendationsController.Recompute)
    ];
    ...
    // silently reopening recommendations to any authenticated caller.
    [Fact]
    public void Controller_RequiresRecommendationsManagePermission()
    {
        var attribute = typeof(RecommendationsController)
    ...
    private static AuthorizationFilterContext AuthorizeAction(string methodName, ClaimsPrincipal user)
    {
        ...
        var context = new AuthorizationFilterContext(
            new ActionContext(
                ...
                {
                    ControllerTypeInfo = typeof(RecommendationsController).GetTypeInfo(),
    ...
    {
        return typeof(RecommendationsController)
