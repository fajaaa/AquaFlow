...
{
    [Fact]
    public async Task RecomputeAsync_ClearIncreasingTrend_CreatesRecommendation()
    {
        await using var context = CreateContext();
        SeedWaterMeter(context, waterMeterId: 1);
        // Two low-low-low-HIGH cycles plus a trailing low: baseline (avg of the last 3 counting
        // readings) is 10, and SSA forecasts the next point as the already-learned HIGH (20) - a
        // clean 2x jump above baseline that is not itself flagged as anomalous, since the last
        // actual reading is just another ordinary "low".
        SeedReadings(context, waterMeterId: 1, consumption: new decimal[] { 10, 10, 10, 20, 10, 10, 10, 20, 10, 10, 10 });
        var service = CreateService(context);

        var created = await service.RecomputeAsync();

        var recommendation = Assert.Single(created);
        Assert.Equal("IncreasingConsumptionForecast", recommendation.Type);
        Assert.False(recommendation.IsRead);
        Assert.Equal(1, recommendation.WaterMeterId);
        Assert.Single(await context.Recommendations.ToListAsync());
    }

    [Fact]
    public async Task RecomputeAsync_CalledAgainWhilePreviousRecommendationUnread_DoesNotCreateDuplicate()
    {
        await using var context = CreateContext();
        SeedWaterMeter(context, waterMeterId: 1);
        SeedReadings(context, waterMeterId: 1, consumption: new decimal[] { 10, 10, 10, 20, 10, 10, 10, 20, 10, 10, 10 });
    ...
    private static RecommendationService CreateService(AquaFlowDbContext context)
    {
        ...
        return new RecommendationService(
            context,
            mapper,
            new IValidator<RecommendationInsertRequest>[] { new RecommendationInsertValidator() },
            Array.Empty<IValidator<RecommendationUpdateRequest>>(),
            Array.Empty<IValidator<RecommendationPatchRequest>>(),
