...
- Account edit (all users): `AccountScreen` shows an "Uredi nalog" card for every role that pushes `AccountEditScreen` (`lib/shared/screens/account_edit_screen.dart`), a form to edit the signed-in user's own email, phone, and first/last name. Email/phone load via `AccountService.fetch()` (`GET /Account/me`, a single `UserResponse` object - not a `PageResult`) and save via `AccountService.update()` (`PUT /Account/me`, body from `AccountDetails.toUpdateJson()` = email + phone only). First/last name lives on `CustomerProfile` instead (not the `User` entity): it loads via `ProfileService.fetchCustomerProfile()` (`GET /CustomerProfiles?UserId=`) and saves via the new `ProfileService.saveName()` (`POST /CustomerProfiles` if the user has no profile yet, else `PATCH /CustomerProfiles/{id}`), sent only when a name was actually entered (`_hasNameInput`, same all-or-nothing pairing as the admin "Moj nalog" screen) - this mirrors `AdminAccountService.saveProfile` but omits `defaultLanguage`/`theme`, which stay untouched on PATCH and default server-side on insert. `AccountService` follows the `CompanySettingsService` template and maps failures to `AccountException`; `ProfileService` maps failures to `ProfileException`. Note: the account screen's email display comes from the JWT (`session.email`), so an email change here is not reflected there until the next login/refresh reissues the token. This is the mobile customer/collector path; the admin desktop "Moj nalog" sidebar item uses a different, richer screen instead (see the next bullet).
